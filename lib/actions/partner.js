...
// Helper to generate a random 6-character hex code
...
export async function getPrimaryUserConnection() {
  ...
  if (data && data.partner_user_id) {
    try {
      const client = await clerkClient();
      const partnerUser = await client.users.getUser(data.partner_user_id);
      data.partner_details = {
        firstName: partnerUser.firstName,
        lastName: partnerUser.lastName,
        imageUrl: partnerUser.imageUrl,
        email: partnerUser.emailAddresses[0]?.emailAddress
      };
    } catch (e) {
...
export async function getPartnerConnection() {
  ...
  if (data && data.primary_user_id) {
    try {
      const client = await clerkClient();
      const primaryUser = await client.users.getUser(data.primary_user_id);
      data.primary_details = {
        firstName: primaryUser.firstName,
        lastName: primaryUser.lastName,
        imageUrl: primaryUser.imageUrl,
        email: primaryUser.emailAddresses[0]?.emailAddress
      };
    } catch (e) {
      console.error("Error fetching primary details:", e);
    }
  }
...
export async function getSharedInsights() {
  ...
  // Fetch primary user's lastActiveAt from Clerk
  let primaryLastActiveAt = null;
  try {
    const client = await clerkClient();
    const primaryUser = await client.users.getUser(primaryUserId);
    primaryLastActiveAt = primaryUser.lastActiveAt;
  } catch (e) {
    console.error("Error fetching primary user activity status:", e);
  }
...
export async function getPrimaryPartnerNudges() {
  ...
  if (connection.partner_user_id) {
    try {
      const client = await clerkClient();
      const partnerUser = await client.users.getUser(connection.partner_user_id);
      partnerLastActiveAt = partnerUser.lastActiveAt;
    } catch (e) {
      console.error("Error fetching partner last active status:", e);
    }
  }
  ...
  try {
    ...
    return { connected: true, nudges: nudges || [], currentUserId: userId, partnerLastActiveAt };
  } catch (err) {
    console.error("Error fetching primary partner nudges:", err);
    return { connected: true, nudges: [], currentUserId: userId, partnerLastActiveAt };
  }
}
