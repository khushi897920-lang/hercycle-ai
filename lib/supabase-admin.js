  ...
  if (process.env.NEXT_PUBLIC_MOCK_DB === 'true') {
    return {
      from: (table) => {
        ...
        if (table === 'audit_log') {
          return makeMockQuery([
            { id: 'mock-audit-12345', user_id: 'mock_user_12345', action: 'DATA_EXPORT', details: {}, created_at: new Date().toISOString() }
          ], null);
        }
        if (table === 'user_drafts') {
          return makeMockQuery([
            { user_id: 'mock_user_12345', draft_type: 'forum_post', title: 'Mock Draft Title', content: 'Mock Draft Content', category_id: 'mock-cat-12345', updated_at: new Date().toISOString() }
          ], null);
        }
